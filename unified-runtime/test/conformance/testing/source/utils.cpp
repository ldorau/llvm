...
GetDeviceMaxWorkItemSizes(ur_device_handle_t device,
  ...
  return GetDeviceVectorInfo<size_t>(device, UR_DEVICE_INFO_MAX_WORK_ITEM_SIZES,
                                     max_work_item_sizes);
}
...
ur_result_t GetDeviceNativeVectorWithInt(ur_device_handle_t device,
                                         uint32_t &vec_width) {
  return GetDeviceInfo<uint32_t>(device, UR_DEVICE_INFO_NATIVE_VECTOR_WIDTH_INT,
                                 vec_width);
}
...
ur_result_t GetDevicePreferredInteropUserSync(ur_device_handle_t device,
                                              bool &sync) {
  return GetDeviceInfo<bool>(device, UR_DEVICE_INFO_PREFERRED_INTEROP_USER_SYNC,
                             sync);
}

ur_result_t GetDeviceParentDevice(ur_device_handle_t device,
                                  ur_device_handle_t &parent) {
  return GetDeviceInfo<ur_device_handle_t>(device, UR_DEVICE_INFO_PARENT_DEVICE,
                                           parent);
}
...
GetDeviceSubGroupIndependentForwardProgress(ur_device_handle_t device,
  ...
  return GetDeviceInfo<bool>(
      device, UR_DEVICE_INFO_SUB_GROUP_INDEPENDENT_FORWARD_PROGRESS, progress);
}
...
ur_result_t GetDeviceGPUSubslicesPerSlice(ur_device_handle_t device,
                                          uint32_t &subslices) {
  return GetDeviceInfo<uint32_t>(device, UR_DEVICE_INFO_GPU_SUBSLICES_PER_SLICE,
                                 subslices);
}
...
ur_result_t MakeUSMAllocationByType(ur_usm_type_t type,
  ...
  case ur_usm_type_t::UR_USM_TYPE_DEVICE:
    return urUSMDeviceAlloc(hContext, hDevice, pUSMDesc, hPool, size, ppMem);
  ...
  case ur_usm_type_t::UR_USM_TYPE_SHARED:
    return urUSMSharedAlloc(hContext, hDevice, pUSMDesc, hPool, size, ppMem);
  }
}
