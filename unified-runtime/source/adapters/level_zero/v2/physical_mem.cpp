...
ur_result_t urIPCPutPhysMemHandleExp(ur_context_handle_t hContext,
  ...
  // Always delete the UR wrapper: urIPCPutPhysMemHandleExp takes ownership of
  // pIPCPhysMemHandleData and the pointer must not be used by the caller after
  // this call, regardless of the return value.  If zeMemPutIpcHandle fails, the
  // driver-side handle may be in an undefined state, but there is no mechanism
  // to retry — the wrapper is freed unconditionally.
  delete HandleData;
